...
export default async function PublicQrPage({
  ...
  return (
    <main className="min-h-screen bg-white pb-9 text-[var(--color-main)] md:pb-12 xl:pb-14">
      ...
      <div className="mx-auto w-full max-w-[375px] px-4 md:max-w-[768px] md:px-5 xl:max-w-[800px]">
        ...
        <section className="rounded-[16px] border-[0.5px] border-[#d6d9de] px-5 py-7 text-center md:px-8 md:py-9">
          ...
          <p className="mx-auto mt-3 max-w-[440px] text-[14px] leading-6 text-[var(--color-muted)] md:text-[16px]">
            {loadFailed
          ...
          <div className="mt-6 flex items-center gap-3 rounded-[16px] border-[0.5px] border-[#d6d9de] px-4 py-4 text-left md:px-5">
            ...
            <div className="min-w-0">
              ...
              <span className="mt-1 block truncate text-[12px] text-[var(--color-muted)] md:text-[14px]">
                {landing?.orderNo
        ...
        {landing && (
          <section className="mt-9 md:mt-11">
            <div className="mb-5">
              ...
              <p className="mb-0 mt-1 text-[14px] text-[var(--color-muted)]">
                {isShopee
                  ? "在蝦皮賣場還有更多商品等你選購"
                  : "喜歡的商品可以透過官方 LINE 詢問"}
              </p>
            ...
            {recommendations.length > 0 ? (
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-4 xl:grid-cols-4">
                {recommendations.map((product) => (
                  <a
                    className={`group relative block min-w-0 overflow-hidden rounded-[16px] border-[0.5px] border-[#d6d9de] bg-transparent text-[var(--color-main)] no-underline transition-[transform,border-color] duration-200 hover:-translate-y-[2px] focus-visible:outline-2 focus-visible:outline-offset-2 ${isShopee ? "hover:border-[#ee4d2d] focus-visible:outline-[#ee4d2d]" : "hover:border-[#06a947] focus-visible:outline-[#06a947]"}`}
                    href={purchaseUrl}
                    key={product.sku}
                    target="_blank"
                    rel="noreferrer noopener"
                    aria-label={`${product.name}，${isShopee ? "前往蝦皮賣場" : "透過官方 LINE 詢問"}`}
                  >
                    ...
                    <h3 className="m-0 line-clamp-2 min-h-[58px] px-[14px] py-3 text-[14px] leading-5 md:min-h-[66px] md:px-4 md:text-[16px] md:leading-6">
                      {product.name}
                    </h3>
                  </a>
                ))}
              ...
              <div className="rounded-[16px] border-[0.5px] border-[#d6d9de] p-6 text-[14px] text-[var(--color-muted)]">
        ...
        >
          ...
          <p className="mx-auto mb-0 mt-2 max-w-[480px] text-[14px] leading-6 text-[var(--color-muted)]">
            {isShopee
          ...
          {purchaseUrl ? (
            ...
            <span className="mt-5 inline-flex min-h-12 w-full items-center justify-center rounded-[16px] border-[0.5px] border-[#d6d9de] px-5 font-semibold text-[var(--color-muted)] md:max-w-[360px]">
              購買入口設定中
            </span>
          )}
        </section>
